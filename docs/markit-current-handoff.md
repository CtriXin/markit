...
当前标准流程已经升级为：Markit 创建 GitLab Work Item 时，必须带上可被 agent 精准读取的项目 metadata、结构化 labels、截图证据；创建成功后可同步到飞书 Base 问题表，按表字段写入项目/域名、问题、GitLab 链接、优先级、状态、备注、附件和负责人。
...
8. GitLab Work Item 是 agent/debug 的 source-backed 入口，body 和 labels 要能区分 project、service、repo、domain、type，不只靠域名。
9. 飞书 Base 是测试/设计/项目同学的表格入口，GitLab 创建后同步一条对应记录，附件里带截图，负责人默认可指定宋鑫。
...
Feishu sync 当前默认关闭；服务器或本地真实同步时显式开启：
...
- `MARKIT_FEISHU_AUTH=auto` 优先用 access token；没有 token 时走本机 `lark-cli`。
- 当前 `lark-cli --as bot` 可以读取 Base 和字段，但创建 record 会返回 `91403 you don't have permission`；默认继续用 `--as user`。
- 飞书 `负责人` / `申请人` 是 user 字段，写 open_id 对象数组，不写 GitLab username。宋鑫当前 open_id 是 `ou_30c6391467af3f8ffb00e07bac50b368`。
...
- 飞书 Base 问题表：GitLab Work Item 创建成功后的协作镜像，默认 app `I7m2bnPDgaYnwksqp1jcmmW9nOd`、table `tbl0yrCubWcpZCvw`、附件字段 `fldKBwIUX2`。
...
- 开启 `MARKIT_FEISHU_SYNC=1` 后，GitLab 新建成功会同步创建飞书 Base record，并上传截图到 `附件`。
...
## GitLab + Feishu Sync Contract

这是以后默认遵守的测试提 Bug 流程契约：

1. Markit 的项目身份来源是 session 里的 `projectSnapshot`，它来自 `ptc-wiki` catalog。GitLab 和飞书同步都以这个 snapshot 为准，不在提交时重新猜项目。
2. GitLab 是 agent/debug 的事实入口。Work Item body 必须包含 `markit.gitlab-issue.v1` metadata，显式写 `Markit Project`、`SCMP Service`、`Business Repo`、`Business Issue Project`、`Bound Domain`、`Current Branch`、`Source URL`、`Final URL`。
3. GitLab labels 必须同时覆盖项目和问题维度：`project:<projectId>`、`service:<scmpService>`、`repo:<businessRepo>`、`domain:<domain>`、`type:<bugTag>`、priority label。保留原始 domain label 只是兼容，不再作为唯一 routing key。
4. 飞书 Base 是 QA / 设计 / 项目同学的协作镜像。GitLab Work Item 新建成功后，Markit 可同步创建一条 Base record，字段映射如下：

```text
域名或模板名称 = <scmpService or project> / <domain>
问题现象 = title + actual + expected + tags
链接 = GitLab Work Item + Final URL + Source URL
优先级 = P0/P1/P2/P3 对应表格 select 文案
项目状态 = 已创建
comment = bug title
备注 = Markit bugId + project + service + repo + branch + GitLab IID + evidence links
附件 = GitLab 已上传的本地截图证据再次上传到飞书附件字段
负责人 = MARKIT_FEISHU_OWNER_OPEN_IDS open_id 数组
```

5. 飞书 user 字段必须写 open_id，不写 GitLab username。`songxin` 是 GitLab username；宋鑫的飞书 open_id 当前是 `ou_30c6391467af3f8ffb00e07bac50b368`。
6. `lark-cli --as bot` 当前实测可读 Base 和字段，但没有 record write 权限，创建记录会返回 `91403`。默认 `MARKIT_FEISHU_CLI_AS=user`；只有确认 bot 被授予 Base 写权限后，才把服务器切到 `bot`。
7. Dedupe 边界：同一个 Markit Bug 已有 `submitted.json` 时不重复创建 GitLab Work Item；因此也不应重复创建新的飞书 record。需要补同步时应做显式 resync 能力，而不是走普通提交。
...
- 完成 `MARKIT_FEISHU_SYNC=1` 后的 GitLab -> Feishu Base record 创建。
- 完成默认 Base/table/attachment field env 配置。
- 完成文本字段映射：`域名或模板名称`、`问题现象`、`链接`、`优先级`、`项目状态`、`comment`、`备注`。
- 完成 `MARKIT_FEISHU_OWNER_OPEN_IDS` 写入飞书 `负责人` user 字段。
- 完成截图证据上传到飞书 `附件` 字段。
- 完成 token auth 和 `lark-cli` auth 两条路径；当前默认 `lark-cli --as user`。
- 实测 bot 可读但不可写，后续需要 Base 权限后再切换。
...
- 还没有 UI 开关和同步状态筛选；当前通过 env 控制。
- 还没有已挂 Bug 的 Feishu record resync / repair 按钮。
- 还没有 bot 写权限；当前 bot record create 实测 `91403`。
- `申请人` 字段当前未默认写入，避免把 bot/user 身份语义写错；需要明确规则后再加。
...
- 把 Feishu sync 默认打开，或把 `MARKIT_FEISHU_CLI_AS` 从 `user` 改成 `bot`。
- 默认开启自动 LLM 调用。
...
11. 如果开启 `MARKIT_FEISHU_SYNC=1`，打开飞书 Base，验证新增 record：
    - `域名或模板名称` 是 `<service/project> / <domain>`。
    - `链接` 包含 GitLab Work Item、Final URL、Source URL。
    - `备注` 包含 Markit bugId、SCMP Service、Business Repo、Branch、GitLab IID。
    - `附件` 有截图文件。
    - `负责人` 是宋鑫或 env 指定 open_id 对应的人。
12. 再次提交同一个 Bug，验证不重复创建，只返回已有链接或同步负责人。
...
- GitLab username 和飞书 open_id 不是一回事；飞书 user 字段写 open_id 对象数组。
- GitLab issue 必须带 `markit.gitlab-issue.v1` metadata 和 project/service/repo/domain/type labels，不能只靠域名。
- Feishu sync 当前默认负责人可用 `MARKIT_FEISHU_OWNER_OPEN_IDS=ou_30c6391467af3f8ffb00e07bac50b368` 指向宋鑫。
- `lark-cli --as bot` 当前只验证可读，record write 被 `91403` 权限拦住；默认不要切 bot。
